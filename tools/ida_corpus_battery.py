...
def stage_targets(args: argparse.Namespace) -> list[dict[str, object]]:
    ...
    for corpus, target in targets:
        ...
        staged.append({
            ...
            "min_output_bytes": int(target.get("minOutputBytes", 64)),
        })
    return staged
...
def build_ida_batch_command(
    ...
    *,
    ...
    if args.paramid:
        cmd.append("--paramid")
    if args.live_callbacks:
        cmd.append("--live-callbacks")
...
def write_input_list(report_dir: Path, mode: str, staged: list[dict[str, object]], min_output_bytes: int = 64) -> Path:
    name = (
        "inputs.txt"
        if mode == "decompile-all" and min_output_bytes == 64
        else f"inputs-{mode}-min{min_output_bytes}.txt"
    )
...
def main() -> int:
    ...
    parser.add_argument("--target-id", default="", help="Comma-separated exact target id filter")
    ...
    parser.add_argument(
        "--live-callbacks",
        action="store_true",
        help="Run without broad batch byte/symbol preloads so IDA callback paths are exercised",
    )
    ...
    for item in staged:
        mode = str(item.get("test_mode") or "decompile-all")
        min_output_bytes = int(item.get("min_output_bytes", 64))
        groups.setdefault((mode, min_output_bytes), []).append(item)

    exit_code = 0
    batch_runs = []
    for (mode, min_output_bytes), mode_targets in groups.items():
        input_list = write_input_list(report_dir, mode, mode_targets, min_output_bytes)
        summary_name = (
            "ida-batch-summary.json"
            if len(groups) == 1 and mode == "decompile-all"
            else f"ida-batch-summary-{mode}-min{min_output_bytes}.json"
        )
