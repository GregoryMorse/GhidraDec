...
def stage_targets(args: argparse.Namespace, output_list: Path) -> list[dict[str, object]]:
    ...
    for corpus, target in targets:
        ...
        if not args.no_stage:
            corpus_tool.download(url, output, args.refresh)
...
def build_ida_batch_command(args: argparse.Namespace, input_list: Path, summary_json: Path) -> list[str]:
    cmd = [
        sys.executable,
...
def main() -> int:
    ...
    input_list = report_dir / "inputs.txt"
    batch_summary = report_dir / "ida-batch-summary.json"
    battery_summary = report_dir / "battery-summary.json"

    staged = stage_targets(args, input_list)
    command = build_ida_batch_command(args, input_list, batch_summary)
    ...
    if args.dry_run:
        ...
        print(f"[ghidradec-battery] running {len(staged)} targets")
        exit_code = subprocess.run(command, cwd=str(ROOT)).returncode
