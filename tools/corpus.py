...
def selected_targets(
    ...
    for corpus in manifest["corpora"]:
        ...
        for target in corpus.get("targets", []):
            if not target_ids and not target.get("enabled", False):
                continue
            if target_ids and target["id"] not in target_ids:
                continue
            if not target_ids and architectures and target["arch"] not in architectures:
                continue
            if not target_ids and tiers and target.get("tier", "") not in tiers:
                continue
...
def stage(args: argparse.Namespace) -> None:
    ...
    target_ids = {value.strip() for value in args.target_id.split(",") if value.strip()} if args.target_id else set()
    targets = selected_targets(manifest, args.corpus, architectures, tiers, target_ids)
    if not targets:
...
def list_targets(args: argparse.Namespace) -> None:
    ...
    tiers = {value.strip() for value in args.tier.split(",") if value.strip()} if args.tier else set()
    target_ids = {value.strip() for value in args.target_id.split(",") if value.strip()} if args.target_id else set()
    for corpus, target in selected_targets(manifest, args.corpus, architectures, tiers, target_ids):
...
def add_common_filters(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--target-id", help="Comma-separated exact target id filter")
