...
def collect_import_eas():
    ...
    def visitor(ea, _name, _ordinal):
        imports.add(int(ea))
        return True

    for index in range(ida_nalt.get_import_module_qty()):
        ida_nalt.enum_import_names(index, visitor)
    return imports


def seed_text_function_if_empty():
    ...
    for index in range(ida_funcs.get_func_qty()):
        ...
        if function is not None and int(function.start_ea) not in import_eas:
    ...
    for index in range(ida_segment.get_segm_qty()):
        ...
        seg_class = ida_segment.get_segm_class(segment) or ""
        is_text = name.lower() == ".text" or seg_class.upper() == "CODE"
        if not is_text or segment.start_ea >= segment.end_ea:
            continue
        if ida_funcs.add_func(segment.start_ea, segment.end_ea):
            log("Seeded function over executable segment {} 0x{:x}-0x{:x}".format(
                name or seg_class or "<unnamed>", segment.start_ea, segment.end_ea))
            ida_auto.auto_wait()
            return
        log("IDA refused to seed function over executable segment {} 0x{:x}-0x{:x}".format(
            name or seg_class or "<unnamed>", segment.start_ea, segment.end_ea))
...
def write_function_list(path):
    ...
    for index in range(ida_funcs.get_func_qty()):
        ...
        functions.append({
            ...
            "is_import": int(function.start_ea) in import_eas,
