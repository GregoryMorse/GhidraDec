...
function(ghidradec_manifest_find_ida_sdk VERSION)
	...
	set(_version_core "${VERSION}")
	if(_version_core MATCHES "^v(.+)$")
		set(_version_core "${CMAKE_MATCH_1}")
	endif()
	set(_tag_suffix "release")
	set(_directory_suffix "")
	if(_version_core MATCHES "^(.+)-(release|sdk\\.[0-9]+)$")
		set(_version_core "${CMAKE_MATCH_1}")
		set(_tag_suffix "${CMAKE_MATCH_2}")
		if(_tag_suffix MATCHES "^sdk\\.([0-9]+)$")
			set(_directory_suffix "sdk${CMAKE_MATCH_1}")
		endif()
	endif()
	if(_version_core MATCHES "^9\\.([0-9]+)(\\.([0-9]+))?$" AND _version_core VERSION_GREATER_EQUAL "9.4")
		set(_minor "${CMAKE_MATCH_1}")
		set(_patch "${CMAKE_MATCH_3}")
		if("${_patch}" STREQUAL "" OR ("${_patch}" STREQUAL "0" AND "${_directory_suffix}" STREQUAL ""))
			set(_directory "idasdk9${_minor}")
			set(_tag_core "9.${_minor}.0")
		elseif("${_patch}" STREQUAL "")
			set(_directory "idasdk9${_minor}${_directory_suffix}")
			set(_tag_core "9.${_minor}.0")
		else()
			set(_directory "idasdk9${_minor}${_patch}${_directory_suffix}")
			set(_tag_core "${_version_core}")
		endif()
		set(GHIDRADEC_IDA_SDK_DIRECTORY "${_directory}" PARENT_SCOPE)
		set(GHIDRADEC_IDA_SDK_ARCHIVE "ida-sdk-v${_tag_core}-${_tag_suffix}.zip" PARENT_SCOPE)
		set(GHIDRADEC_IDA_SDK_SOURCE "public-release" PARENT_SCOPE)
		set(GHIDRADEC_IDA_SDK_CMAKE_VARIABLE "IDA_SDK_DIR" PARENT_SCOPE)
		set(GHIDRADEC_IDA_SDK_FOUND TRUE PARENT_SCOPE)
		return()
	endif()
...
function(ghidradec_manifest_resolve_ida_sdk VERSION)
	...
	set(_candidate_roots "${CMAKE_SOURCE_DIR}/.idasdks")
	if(DEFINED ENV{GHIDRADEC_IDA_SDK_CACHE} AND NOT "$ENV{GHIDRADEC_IDA_SDK_CACHE}" STREQUAL "")
		list(APPEND _candidate_roots "$ENV{GHIDRADEC_IDA_SDK_CACHE}")
	endif()
