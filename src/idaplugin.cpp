...
bool GhidraDec::canDecompileInput()
{
...
#endif
	{
		...
		{
			qstring asyncRegression;
			const bool runAsyncRegression =
				qgetenv("GHIDRADEC_TEST_ASYNC", &asyncRegression) &&
				std::string(asyncRegression.c_str()) != "0";
			qstring liveCallbacksEnv;
			const bool liveCallbackRegression =
				qgetenv("GHIDRADEC_TEST_LIVE_CALLBACKS", &liveCallbacksEnv) &&
				std::string(liveCallbacksEnv.c_str()) != "0";
			...
			for (unsigned i = 0; i < get_func_qty(); ++i)
			{
				...
				if (cmt.find("<ghidradec_select>") != std::string::npos)
				{
					...
					decompInfo->setIsUseThreads(liveCallbackRegression ? false : runAsyncRegression);
		...
		else if (arg == 5)
		{
			...
			decompInfo->setIsUseThreads(!liveCallbackRegression);
			decompInfo->skipParamIdentification = skipRegressionParamId;
			decompInfo->pm->runAllDecompilation();
		}
		else
...
ssize_t idaapi GhidraDec::on_event(ssize_t notification_code, va_list va)
{
	if (notification_code == ui_populating_widget_popup)
		return ui_callback(decompInfo, static_cast<int>(notification_code), va);
#else
