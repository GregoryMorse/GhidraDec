...
{
	return getWordGlobal(word, color) != nullptr;
}
...
{
	return getWordFunction(word, color) != nullptr;
}
...
bool idaapi GhidraDec::moveToPrevious()
{
			...
			&& decompInfo->navigationActual != decompInfo->navigationList.begin())
	{
		...
		show.execute();
		displayBlockGraph(decompInfo, fit->first->start_ea);
		return false;
	}
...
bool idaapi GhidraDec::insertCurrentFunctionComment()
{
			...
			MAXSTR))
	{
		...
		ShowOutput show(decompInfo);
		show.execute();
		displayBlockGraph(decompInfo, fnc->start_ea);
...
bool idaapi ct_double(TWidget* cv, int shift, void* ud)
{
	...
	if (di->pm->getWordFunction(word, color) != nullptr
			|| di->pm->getWordGlobal(word, color) != nullptr)
	{
