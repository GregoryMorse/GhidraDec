...
static void traceIface(const std::string& message)
{
	std::cerr << "[GhidraDecIface] " << message << std::endl;
}
...
public:
	...
	void launchDecompiler()
	{
		...
		HANDLE hNullErr = CreateFileA("NUL", GENERIC_WRITE, FILE_SHARE_WRITE, &saAttr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		si.hStdError = hNullErr != INVALID_HANDLE_VALUE ? hNullErr : GetStdHandle(STD_ERROR_HANDLE);
		...
		*handleOut = wpipefd[1];
		...
		void* p;
#if defined(_WINDOWS)
		if (!CreateProcessA(cmd.c_str(), NULL, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi))
			traceIface("CreateProcessA failed: " + std::to_string(GetLastError()));
		p = pi.hProcess;
	...
	{
		return _read(rdHandle, Buf, (unsigned int)MaxCharCount);
	}
	...
	{
		const uintb base = 0x80483b4;
		const size_t progSize = sizeof(myprog);
		for (int i = 0; i < size; i++) {
			uintb cur = addr.offset + i;
			ptr[i] = (cur >= base && (cur - base) < progSize) ? myprog[cur - base] : 0;
		}
		return size;
	}
	...
	void getMappedSymbol(AddrInfo addr, MappedSymbolInfo& msi)
	{
		const uintb base = 0x80483b4;
		const size_t progSize = sizeof(myprog);
		if (addr.space != "ram" || addr.offset < base || addr.offset >= base + progSize)
			return;

		msi.kind = KIND_FUNCTION;
		msi.readonly = true;
		msi.volatil = false;
		msi.name = "test_function";
		msi.entryPoint = base;
		msi.size = progSize;
		msi.ranges.push_back(RangeInfo{ "ram", base, base + progSize - 1 });
		msi.func = {};
		msi.func.extraPop = (unsigned long long)-1;
		msi.func.model = "default";
		msi.func.retType.argIndex = -1;
		msi.func.retType.addr.size = 0;
		msi.func.retType.pi.ti.push_back(TypeInfo{ "void", 0, "void" });
	}
	...
	{
		if ((data.find("command_get") != std::string::npos &&
			data.find("command_getregister") == std::string::npos &&
			data.find("packedPcode") == std::string::npos) ||
			data.rfind("exception", 0) == 0 ||
			data.rfind("command(\"decompileAt", 0) == 0)
			traceIface(std::string(bWrite ? "<- " : "-> ") + data);
	}
};
...
int __cdecl main()
{
...
#ifdef _WINDOWS
	char buf[256];
	if (GetEnvironmentVariableA("GHIDRA_INSTALL_DIR", buf, 256))
		ghidraPath = std::string(buf);
	if (!ghidraPath.empty() && ghidraPath.back() != '/' && ghidraPath.back() != '\\')
		ghidraPath += "/";
#endif
	traceIface("GHIDRA_INSTALL_DIR=" + ghidraPath);
	std::vector<LangInfo> li;
	std::map<std::string, std::vector<int>> toolMap;
	traceIface("loading language files");
	DecompInterface::getLangFiles(ghidraPath + "Ghidra/Processors/", "IDA-PRO", toolMap, li);
	traceIface("language files loaded: " + std::to_string(li.size()));
	...
	traceIface("register x86gcc");
	di.registerProgram();
	traceIface("decompile x86gcc");
	cout << di.doDecompile(defaultDecMode, AddrInfo{ "ram", 0x80483b4 }, display, funcProto, funcColorProto, paramInfo, blockGraph);
	di.deregisterProgram();
	delete cb;
	traceIface("done");
	return 0;
	} catch (const DecompError& err) {
		traceIface("DecompError: " + err.explain);
		return 1;
	} catch (const LowlevelError& err) {
		traceIface("LowlevelError: " + err.explain);
		return 1;
	} catch (const XmlError& err) {
		traceIface("XmlError: " + err.explain);
		return 1;
	} catch (const std::exception& err) {
		traceIface(std::string("std::exception: ") + err.what());
		return 1;
	}
}
