...
struct Options
{
	...
	bool noCastPrinting;
	bool hideExtensions;
	bool analyzeForLoops;
	bool allowContextSet;
	bool errorUnimplemented;
	bool errorReinterpreted;
	bool errorTooManyInstructions;
	bool jumpLoad;
	int maxLineWidth;
	int maxJumpTableSize;
	int maxInstructions;
	int indentIncrement;
	int commentIndent;
	...
	std::string aliasBlock; //"array"
	std::string namespaceStrategy; //"minimal"
	std::string splitDatatype1; //"struct"
	std::string splitDatatype2; //"array"
	std::string splitDatatype3; //"pointer"
	std::string nanIgnore; //"compare"
	std::string braceFormatFunction; //"skip"
	std::string braceFormatIfElse; //"same"
	std::string braceFormatLoop; //"same"
	std::string braceFormatSwitch; //"same"
};
...
using ghidra::ContextInternal;
using ghidra::Element;
using ghidra::Sleigh;
using ghidra::VarnodeTpl;
class XmlPcodeEmit;
...
class DecompInterface
{
	...
	std::string packedOptions; // Current decompiler options encoded for Ghidra 12+ decompile.exe
	std::string callStyle;
	std::map<std::string, unsigned long long> callStyles;
	std::map<std::string, unsigned long long> symbolIds;
	std::map<std::string, unsigned long long> functionSymbolEntries;
	struct ScopedSymbolName
	{
		unsigned long long scopeId = 0;
		std::string name;
	};
	std::map<unsigned long long, std::vector<std::string>> namespacePaths;
	std::map<std::string, unsigned long long> namespaceIdsByPath;
	std::map<unsigned long long, std::map<std::string, bool>> namespaceNames;
	unsigned long long nextNamespaceId = 1;
	...
	std::string getPackedOptions(Options opt);
	std::string getPackedAddress(AddrInfo addr);
	...
	std::string getFunctionSymbolName(const std::string& name, unsigned long long entryPoint);
	ScopedSymbolName getScopedSymbolName(const std::string& name, const std::string& parentname = "");
	unsigned long long registerNamespacePath(const std::vector<std::string>& path);
	void registerNamespaceName(unsigned long long scopeId, const std::string& name);
	std::string getPackedNamespacePath(unsigned long long namespaceId);
	bool isNameUsedInNamespaces(const std::string& name, unsigned long long firstId, unsigned long long lastId);
protected:
